...
def add_student():
    ...
    if request.method == 'POST':
            ...
            if filename.split(".")[1] not in ALLOWED_EXTENSIONS:
                    flash("Image formate allowed - <rollno/name>.png or jpg or jpeg (eg.20d4001.jpg)")
                    return redirect(url_for('Index'))
            else:
...
def update_student(roll):
    ...
    if request.method == 'POST':
        ...
        dept = request.form['dept'].upper()
        contact = request.form['contact']
        path = 'C:/Users/kiran/Desktop/project/crud-2/static/uploads/'
        file = request.files['file']

        # if (oldRoll != roll):
        #     # check whether the new roll already exist ---> if exist throw error
        #     cur.execute(f"SELECT * FROM students where roll='{roll}'")
        #     if cur.fetchone() != None:
        #         flash(f"{roll}-already exist(update FAILED!) try different ROLL.NO")
        #         return redirect(f'/edit/{oldRoll}')
        ...
        if len(file.filename)==0:
            ...
            print("FILENAME: " + filename)
             ...
             print(
                 "^^^^^^^^^^^^^^^^^^^^^^^^^^^ uploaded image checking for existance ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
             print(matches)
             print(faceDis)
             print(matchIndex)

             if matches[matchIndex]:
                 flash(f"{classNames[matchIndex]}-Image already exist\n(update FAILED!)")
                 return redirect(f'/edit/{oldRoll}')
             else:
                 print(roll)

                 cur.execute(f"SELECT image from students where roll='{oldRoll}'")
        ...
        print(sname,dept,contact,filename,roll)
